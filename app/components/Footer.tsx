...
export default function Footer() {
  ...
  return (
    <footer className="bg-white text-white font-sans relative overflow-hidden">
      {/* Main Footer Content */}
      <div className="relative z-10 px-8 md:px-16 pt-16 pb-">
          ...
          <div className="flex flex-col gap-3">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.href}
                className="text-black text-xl font-medium tracking-wide hover:text-[#71c6a4] transition-colors"
              >
                {link.name}
              </Link>
            ))}
        ...
        <div className="flex items-center gap-30">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div className="flex flex-col gap-1">
              <p className="text-black text-xl"> 2026 ALL RIGHTS RESERVED</p>
              <Link href="#" className="text-black/40 text-xl hover:text-[#71c6a4] transition-colors">PRIVACY POLICY</Link>
            </div>
          </div>
          <div className="flex justify-center gap-4 my-12">
            <a href="#" className="w-16 h-16 rounded-full bg-black/10 flex items-center justify-center hover:bg-[#71c6a4] transition-colors">
