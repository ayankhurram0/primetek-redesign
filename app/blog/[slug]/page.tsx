...
export default function BlogInnerPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="flex min-h-screen flex-col overflow-x-hidden bg-transparent">
      ...
      <section className="relative pt-40 pb-20 bg-white/60 backdrop-blur-sm border-b border-ink/10">
        <div className="max-w-4xl mx-auto px-6 md:px-12 text-center">
          <StaggerContainer delayChildren={0.2} staggerChildren={0.15}>
            <StaggerItem>
              <div className="flex items-center justify-center gap-2 text-sm font-bold tracking-widest uppercase mb-6 text-accent">
            ...
            <StaggerItem>
              <h1 className="text-4xl md:text-6xl font-bold leading-tight mb-8 tracking-tight text-[#2b4c8c] max-w-4xl mx-auto">
                {post.title}
              </h1>
            </StaggerItem>

            <StaggerItem>
              <div className="flex items-center justify-center gap-6 text-ink-subtle font-medium text-sm md:text-base">
                <div className="flex items-center gap-2">
                  <svg className="w-5 h-5 text-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
          ...
          <AnimationWrapper direction="up" distance={30}>
            ...
            <article
              className="prose prose-lg md:prose-xl prose-slate max-w-none 
                         prose-headings:font-bold prose-headings:text-[#2b4c8c] prose-headings:mb-6
                         prose-h2:text-3xl prose-h2:mt-12
                         prose-p:font-light prose-p:text-ink-subtle prose-p:leading-relaxed prose-p:mb-8
                         prose-a:text-accent prose-a:font-bold hover:prose-a:text-[#2b4c8c]
                         prose-strong:text-[#2b4c8c] prose-strong:font-bold
                         prose-li:text-ink-subtle prose-li:font-light
          ...
          <AnimationWrapper direction="up" distance={20} className="mt-12">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
              <div className="flex items-center gap-3">
                <span className="text-sm font-bold text-ink-muted uppercase tracking-widest">Share Article:</span>
                {/* Placeholder social buttons */}
                <button className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-ink-subtle hover:bg-[#2b4c8c] hover:text-ink transition-colors">
              ...
              <Link href="/blog" className="px-6 py-3 bg-teal-400/10 text-[#2b4c8c] font-bold uppercase tracking-wider text-xs rounded-full hover:bg-teal-400 hover:text-ink transition-colors">
