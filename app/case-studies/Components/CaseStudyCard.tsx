      ...
      >
         ...
         <div className={`px-10 py-4 border-b border-white/5 flex items-center justify-between ${styles.bg}`}>
            <div className="flex items-center gap-6">
               <span className={`flex items-center gap-2 text-xl font-poppins font-bold uppercase tracking-[0.3em] ${styles.text}`}>
                  <Hash className="w-4 h-4" /> STUDY LOG 0{idx + 1}
               </span>
               <span className="hidden sm:block w-px h-6 bg-white/10" />
               <span className="hidden sm:block text-xl text-slate-500 font-poppins font-bold uppercase tracking-widest">{study.type}</span>
            </div>
            <div className={`px-4 py-2 rounded-sm border border-current text-xl font-poppins font-bold uppercase tracking-[0.2em] ${styles.text}`}>
               {study.category}
            </div>
         </div>
         ...
         <div className="p-10 md:p-16">
            ...
            <div className="flex flex-col lg:flex-row gap-16 mb-16">
               <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-6 mb-8">
                     <div className="flex items-center gap-2 text-lg font-poppins text-slate-500 font-bold uppercase tracking-widest">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-12 pt-10 border-t border-white/5">
                     <div className="space-y-4">
                        <h4 className={`text-xl font-poppins font-bold uppercase mb-4 ${styles.text}`}>INITIAL CONDITION</h4>
                        <p className="text-slate-400 text-xl leading-relaxed font-light">{study.challenge}</p>
                     </div>
                     <div className="space-y-4">
                        <h4 className={`text-xl font-poppins font-bold uppercase mb-4 ${styles.text}`}>SYSTEM DEPLOYMENT</h4>
               ...
               <div className="lg:w-80 shrink-0">
                  <div className="sticky top-10">
                     <div className={`p-10 border border-white/10 ${styles.bg} relative overflow-hidden`}>
                        ...
                        <div className="text-xl text-white font-poppins font-bold uppercase mb-4">{study.statLabel}</div>
                        ...
                        <ul className="space-y-4">
                           {study.results.map((result, i) => (
                              <li key={i} className="flex items-start gap-3 group/li">
                                 ...
                                 <span className="text-slate-300 font-poppins text-lg leading-tight uppercase opacity-80">{result}</span>
                              </li>
                           ))}
                        </ul>
                     </div>
            ...
            <div className="bg-white/[0.02] border border-white/5 p-12 relative overflow-hidden">
               ...
               <div className={`flex items-center gap-6 relative z-10 border-l-2 pl-6 ${styles.text.replace('text-', 'border-')}`}>
                  <div>
                     <div className="text-white font-poppins font-bold text-2xl uppercase tracking-widest">{study.author}</div>
                     <div className="text-2xl text-slate-500 font-poppins font-bold uppercase tracking-widest mt-1 opacity-60 font-bold">{study.authorRole}</div>
                  </div>
               </div>
            </div>
         </div>
